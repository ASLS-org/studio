...
class Controls {
  ...
  constructor() {
    if (!controlsInstance) { //Ensuring singlelessness
  ...
  init(camera, el, orbitcontrolsControlsHandle) {
    ...
    this.handle.translationSnap = .5 //Setting default handle translation snap
  ...
  setFocus(state) {
    ...
    let startTPos = this.controlHandle.target.clone();
    let endPos = state ? this.groupedInstances.position.clone() : DEFAULT_ZOOM_OUT_ENDPOS;
    let startTime = performance.now();

    let dX = (endPos.x - startPos.x);
    let dY = state ? 0 : (endPos.y - startPos.y);
    let dZ = state ? ((endPos.z - startPos.z) - 0) : (endPos.z - startPos.z);

    let dTX = state ? (endPos.x - startTPos.x) : -startTPos.x;
    let dTY = state ? (endPos.y - startTPos.y) : -startTPos.y;
    let dTZ = state ? (endPos.z - startTPos.z) : -startTPos.z;

    const animationFunction = () => {
      let time = performance.now() - startTime;
      let animationPercentage = Math.sin(((time / this.focusTransitionDuration) * Math.PI) / 2);
   ...
   */
  applyTransformation() {
    for (var i = this.groupedInstances.children.length - 1; i >= 0; i--) {
      let child = this.groupedInstances.children[i];
      child.updateMatrixWorld();
      child.getWorldPosition(position)
      child.getWorldQuaternion(quaternion);
      this.groupedInstances.remove(child)
      SceneManager.add(child)
      let instanceHandle = this.pooledInstances.find(i => i._3DModel._dummy === child)
      if (instanceHandle) {
  ...
  showHelpers() {
    ...
    if (this.pooledInstances.length) {
      ...
      let bbW = (this.boundingBox.max.x - this.boundingBox.min.x);
      let bbH = (this.boundingBox.max.y - this.boundingBox.min.y);
      let bbD = (this.boundingBox.max.z - this.boundingBox.min.z);
  ...
  clearAllPooledInstances() {
    for (var i = this.pooledInstances.length - 1; i >= 0; i--) {
  ...
  clearPooledInstance(instance) {
    let index = this.pooledInstances.findIndex(i => i == instance);
    if (index > -1) {
