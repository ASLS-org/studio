...
class Fixture extends Proxify {
    ...
    if (this.hasColor) {
      this.quickChannelsAccessors.Color.forEach(channel => {
        if (RGB_CHANNELS.includes(channel.color)) {
          let value = rgbValue[RGB_CHANNELS.indexOf(channel.color)]
          this.setChannel(channel.id - 1, value);
        } else if (CMY_CHANNELS.includes(channel.color)) {
          let value = 255 - rgbValue[CMY_CHANNELS.indexOf(channel.color)]
          this.setChannel(channel.id - 1, value);
        }
      })
    }
  ...
  get modeIndex() {
    let modeIndex = this.modeNames.indexOf(this.modeName);
   ...
   */
  setChannel(id, value) {
    let channel = this.channels[id]; //Getting channel instance from ID
    ...
    let capability = channel.getCapability(value); //Fetching channel's capability from value
    ...
    if (capability) { //Making sure channel's capability is defined
      let values = capability.getValue(value); //Fetching values from capability value
      switch (capability.type) { //Checking capability type
        ...
        case CAPABILITY_TYPES.WheelSlot: { //Capability is wheelSlot
          let slot = Math.floor(capability.getValue(value).slotNumber) - 1.0; //Parsing slot number from value
  ...
  prepare3DModelInstance() {
    switch (this.category) { //Checking fixture category
      case FIXTURE_TYPES.MOVING_HEAD: { //Fixture is a moving head
        let movingHead = new MovingHead({ //Creating new moving head instance  
  ...
  prepareChannels() {
    ...
    this.mode.channels.forEach(channel => { //Looping through channel modes
      if (channel && typeof channel === 'string') { //Making sure the OFL mode data contains a channel value
        ...
        let chn = new Channel({ //Instanciating new channel
    ...
    this.channels.forEach(channel => {
      if (channel.fineChannelAliases && !channel.isFine) {
        channel.fineChannels = channel.fineChannelAliases.flatMap(alias => {
          let standardizedAlias = alias.replace(' ', '').toLowerCase();
          let fineChannel = this.channels.find(channel => channel.type.replace(' ', '').toLowerCase() === standardizedAlias);
          return fineChannel || []
        })
      }
    })
